...
const APP_NAME = "AI Usage.app";
const BUNDLE_ID = "com.muhammed.ai-usage-menu";
const SOURCE_ROOT = import.meta.dir;

export async function installAIUsageMenu(): Promise<void> {
  const home = homedir();
  const appPath = join(home, "Applications", APP_NAME);
  const executablePath = join(appPath, "Contents", "MacOS", "AIUsageMenu");
  ...
  await execa(
    "swift",
    ["build", "-c", "release", "--package-path", SOURCE_ROOT],
    {
      stdio: "pipe",
    },
  );
  const signingIdentity = await findAppleDevelopmentIdentity();
  const stagingRoot = await mkdtemp(join(tmpdir(), "ai-usage-menu-"));
  const stagedApp = join(stagingRoot, APP_NAME);
  try {
    await mkdir(join(stagedApp, "Contents", "MacOS"), { recursive: true });
    await copyFile(
      join(SOURCE_ROOT, ".build", "release", "AIUsageMenu"),
      join(stagedApp, "Contents", "MacOS", "AIUsageMenu"),
    );
    await copyFile(
      join(SOURCE_ROOT, "Info.plist"),
      join(stagedApp, "Contents", "Info.plist"),
    );
    await chmod(join(stagedApp, "Contents", "MacOS", "AIUsageMenu"), 0o755);
    await execa(
      "/usr/bin/codesign",
  ...
  if (userID === undefined)
    throw new Error(
      "The AI Usage menu-bar app can only be installed on macOS.",
    );
  ...
  await execa("/bin/launchctl", ["bootstrap", domain, launchAgentPath], {
    stdio: "pipe",
  });
}
...
async function findAppleDevelopmentIdentity(): Promise<string> {
  ...
  const identity = stdout.match(
    /^\s*\d+\)\s+([A-F0-9]{40})\s+"Apple Development:/m,
  )?.[1];
  if (!identity) {
    throw new Error(
      "A valid Apple Development signing identity is required to install AI Usage.",
    );
  }
  return identity;
}
