...
- **Repair:** Create or correct the identifier, key, and return URLs in the Apple developer account; store private key files under `$SERVICE_CREDENTIALS_HOME/apple-sign-in/` only when local agent access needs them, otherwise keep them in runtime secret storage.
