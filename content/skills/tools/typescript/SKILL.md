...
- Do not wrap local async calls in `try`/`catch` just to return `null`, `false`, or an empty collection. Let existing handlers handle failures unless this layer can recover.
