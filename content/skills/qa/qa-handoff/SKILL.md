...
3. Confirm the local app is usable through the repo's allowed project flow, using the project's dev-server manager or documented commands, and confirm local migrations have run when the QA target depends on database schema or seeded state.
...
5. For web handoffs, identify the canonical checkout or secondary worktree, then discover the active dev server and base URL from the repo's supported project flow instead of composing one from memory.
6. If the needed clone, worktree, or dev server setup was removed, recreate it through the repo's supported project flow and complete setup before continuing.
7. Confirm the relevant web process is running in the exact checkout, then verify the exact URL and starting page with a browser load, `curl`, or a project health check before sharing it.
...
11. Write short, basic manual cases the user can run without guessing.
