...
4. Treat runnable access as a handoff gate: do not hand over a URL, device, or starting state until it has been opened successfully in the target environment.
...
7. Confirm DDEV is running in the exact checkout, then share a clickable responding DDEV URL from there; never substitute the canonical URL for a worktree.
...
- Include negative, edge, permission, and integration cases only when realistic.
