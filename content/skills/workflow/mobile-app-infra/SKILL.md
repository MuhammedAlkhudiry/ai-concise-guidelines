...
Run `scripts/mobile-store-status.ts --help` for its current interface and use it only for read-only release/status checks. Never print tokens, private keys, service-account JSON, signed purchase payloads, or private account data.
Use $service-access to discover, verify, or repair App Store Connect, Google Play, Google Cloud, and Sign in with Apple access before asking the user for credentials or explaining credential storage.
