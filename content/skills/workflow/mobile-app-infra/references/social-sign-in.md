...
1. Map the provider, platform, auth library, app identifier, build profile, backend exchange, account-linking policy, and exact signed artifact under
   test.
2. Trace the complete contract from the button through provider result, backend verification, account decision, app session, persistence, sign-out,
...
4. Fix the broken boundary before changing the flow. Rebuild after native dependency, plugin, identifier, capability, entitlement, profile, signing,
   service-file, or build-time configuration changes.
5. Verify each changed provider on the signed artifacts users receive. Provider UI opening is insufficient; prove backend acceptance, account
   selection or linking, persisted app session, cancellation, rejection, collision, sign-out, and retry.
