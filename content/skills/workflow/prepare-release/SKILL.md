...
2. Run or verify the checks that match the changed surface, and use their result to determine whether the release is ready, not ready, or ready with named caveats.
...
5. Translate the release into product effects: how users use it, how stored data changes, and what
   support, QA, analytics, or monitoring is needed after launch. Use concrete relevant examples such
   as seeders, backfills, derived-store refreshes, forced logouts, restarts, provider dashboards,
   notification templates, or watches.
6. Add scheduled cleanup only when the release creates temporary scaffolding.
   ...
   For requested post-release automation, create actionable watches across user behavior, data
   integrity, background work, product health, and cleanup; each needs cadence, stop condition,
   report signal, and action.
7. Lead with the release verdict and group only populated sections: blockers, release actions, product effects, checks, automation, rollback, and cleanup.
