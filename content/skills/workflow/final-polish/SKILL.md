...
3. Run $code-simplifier in self-contained subagents with no inherited turns. Pass the exact scope, diff target, and constraints. Evaluate each pass
   before starting another; only these subagents may edit.
...
10. Report `READY FOR HUMAN REVIEW` only when no known blockers remain. Present $code-review, final reviewer-gate, and $refactor-opportunities
    findings as proposed follow-up, not silent implementation. Give every pass its own section and state empty results explicitly:
