...
1. Run `system-tools status`, then `system-tools update-plan`. Treat their live output as the inventory, ownership, status, and supported-update
   authority.
...
3. Re-run `system-tools status` until every eligible item is current or has an exact blocker.
