...
4. Keep the existing PHP execution boundary: run PHP and Composer through the project's established local flow, and use Herd for Laravel/PHP commands when the repo requires it.
...
6. Apply straightforward fixes directly: renamed classes, namespace changes, method signature updates, config key changes, type errors, tool config updates, and simple test assertion updates.
