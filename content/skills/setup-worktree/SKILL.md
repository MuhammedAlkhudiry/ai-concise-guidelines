...
Create a project script, usually `scripts/setup-worktree.ts`, that takes a fresh worktree path and makes the web/backend app ready for useful work quickly.
...
3. Before editing, identify the full web worktree setup path: worktree creation command, app directories, package manager, DDEV root/docroot, reusable dependency/env/data/service state, URLs, QA fixture, and first useful web/backend check command.
4. Read root `AGENTS.md` and root `QA.md` first when they exist, then nested `AGENTS.md`, package manifests, DDEV config, `.env.example`, and `CHECKLIST.md`.
5. Create or update `scripts/setup-worktree.ts`. The script should accept a fresh worktree path and support `--dry-run`.
6. Make the script fix soft blockers when repo evidence is clear and safe: dependency reuse, env reuse, per-worktree DDEV lane setup, MinIO/local S3 setup, DDEV health checks, service checks, reusable DB fixture checks, and first web/backend checks.
...
- Separate root commands from backend, web, and mobile app directory commands.
- Prefer explicit commands from root `CHECKLIST.md`, `QA.md`, and `AGENTS.md` over collector-guessed checks.
- A 1-minute web start means another agent can create a fresh worktree, run the setup script, and begin useful web/backend code or checks within 1 minute when the canonical checkout has warm reusable state.
...
- For ignored dependency directories such as `vendor` and `node_modules`, prefer a fast reuse path from the canonical checkout when lockfiles match. Use symlinks for local worktrees when safe; otherwise give the install command as a hard blocker for the 1-minute target.
...
- If realistic local data is needed, mention `~/db-dumps` and the `prod-db-to-ddev` skill, but never import production data without explicit approval.
