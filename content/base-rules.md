...
- **SIMPLE** — Simple is absolute in everything: logic, tools, code, structure, and workflow. Fight ruthlessly against creeping or sneaky complexity, keep solutions as simple as they can be, and when complexity is not clearly required, add a short `Complexity Warning` section that calls it out and says what simpler path should be considered. Skip it when there is no meaningful complexity risk.
...
- **PLAN-ROLL-OUT** — Mention the next phase or immediate next steps, required production rollout work, and deployable status only when finishing a plan or other significant work. Include rollout items such as DB changes, migrations, commands, environment/config updates, or other deployment steps only when they still apply.
...
- **RETHINKING-NEEDED** — When progress is stuck, the problem likely needs reframing, or a zoomed-out view would help, add a short `Rethinking Needed` section that says what assumption, framing, or direction should be reconsidered. Skip it when no rethink is needed.
