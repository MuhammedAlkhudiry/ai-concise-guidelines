...
**You MUST maintain discussion state across turns.** Memory is unreliable—use artifacts.

1. **On first discussion**: Create `.windsurf/discussions/<topic-name>.discuss.md` with:
   - Topic, date, participants' positions
   - Key arguments (for/against)
   - Decisions reached
   - Open tensions

2. **On every subsequent turn**:
   - **FIRST**: Read the active discussion file before responding
   - Append new arguments, update positions, record decisions
   - Reference prior points by number when building on them

3. **Never rely on memory alone**. If unsure about prior context, read the discussion file.

4. **Discussion file format**:
   ```markdown
   # Discussion: <topic>
   Started: YYYY-MM-DD | Status: open|concluded
   
   ## Current Understanding
   [Sharpest restatement of the idea/problem]
   
   ## Arguments
   ### For
   1. Argument (strength: high/medium/low)
   
   ### Against  
   1. Counter-argument (addressed: yes/no)
   
   ## Decisions
   - [YYYY-MM-DD] Decision: rationale
   
   ## Open Tensions
   - Unresolved trade-off or uncertainty
   
   ## Next Steps
   - Concrete action or question to resolve
   ```
