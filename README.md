...
Managed skills are synced in `~/.agents/skills` during install. Local skills come from `content/skills`, while remote skills are declared in `config/skills.ts` and fetched fresh from GitHub during install. Removed managed skills are pruned, while existing custom skills on the device are preserved. You can run quick non-interactive prompts with `opencode run` once a provider is configured.
